...
<template>
  <div class="resume-label">
    ...
    <van-swipe-cell v-if="store.companyList.length" v-for="(item, index) in store.companyList" :key="index">
      <h4 @click="editWork(item)">{{ item.company_name }}<van-icon v-if="props.page !== 'preview'" name="arrow" /></h4>
      <h5>{{ item.work_position }}</h5>
      <p>{{ item.start_time }} - {{ item.end_time }}</p>
      <h5>工作描述</h5>
      <van-field v-model="item.company_describe" autosize readonly label="" type="textarea" />
      <template #right>
        <div class="img">
          <img v-if="props.page !== 'preview'" @click="submitDelete(item.id)" src="@/assets/img/icon/icon-delete.png"
            class="delete" />
        </div>
      </template>
    </van-swipe-cell>
